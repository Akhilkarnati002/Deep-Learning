...
- Generator: ResNet-style (similar to CycleGAN/CUT), 1-channel (IR).
- Discriminator: PatchGAN-style critic with WGAN-GP.
- Losses:
...
class ResNetGenerator(nn.Module):
    """
    ResNet-based generator (similar to CycleGAN/CUT) for 1-channel IR images.

    Input:  (B, 1, H, W)
    Output: (B, 1, H, W) in [-1, 1]
    """

    def __init__(self, in_channels=1, out_channels=1, num_filters=64, num_blocks=6):
        super().__init__()
...
# -----------------------------
# Model: Discriminator (PatchGAN Critic)
# -----------------------------
...
def pseudo_low_from_high(high, scale=2):
    ...
    Approximate the low-res camera by:
      1) average pooling with stride=scale
      2) upsampling back to original size (bilinear)
    """
    # blur
    high_blur = F.gaussian_blur(high, kernel_size=7, sigma=1.5)
    # downsample
...
def train(
    ...
    D = PatchGANDiscriminator(in_channels=1, num_filters=64).to(device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch in dataloader:
            ...
            for _ in range(n_critic):
                ...
                score_real = D(real_high)
                score_fake = D(fake_high)
            ...
            fake_high = G(low)
            score_fake = D(fake_high)
            adv_loss = -score_fake.mean()  # want critic to think G(low) is real
            ...
            # Identity loss: G(high) ~ high (stabilizes mapping)
            id_high = G(real_high)
            ...
            def gradient(x):
                ...
                dy = x[:, :, 1:, :] - x[:, :, :-1, :]
                return dx, dy
            dx_fake, dy_fake = gradient(pseudo_low)
            dx_low,  dy_low  = gradient(low)
            grad_loss = l1(dx_fake, dx_low) + l1(dy_fake, dy_low)

            lambda_grad = 5.0  # start small

            loss_G = adv_loss + lambda_recon * recon_loss + lambda_id * id_loss + lambda_grad * grad_loss
            ...
            if global_step % log_interval == 0:
                print(
                    ...
                    f"(adv: {adv_loss.item():.4f}, recon: {recon_loss.item():.4f}, id: {id_loss.item():.4f})"
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Unpaired IR WGAN-GP (low -> high) with downsampling consistency"
    )
