...
class CUTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # List of layer indices in generator to use
        ...
        self.loss_names.append('L1')
        self.loss_names.append('FM')


        # Hyperparameters
        self.lambda_GAN = float(self.opt.get('lambda_GAN', 1.0))
        self.lambda_idt = float(self.opt.get('lambda_idt', 0.0)) if self.use_simplified else 0.0
        self.lambda_L1 = float(self.opt.get('lambda_L1', 5.0))   
        self.lambda_FM = float(self.opt.get('lambda_FM', 2.0))
        self.lambda_FM = float(self.opt.get('lambda_FM', 0.0))
        if self.lambda_FM > 0:
            self.loss_names.append('FM')
        ...
        training_lr = train_opt.get('lr', 1e-4)
        training_lr_D = train_opt.get('lr_D', 1e-5)
    ...
    # NEVER let D’s loss backprop into G
        fake_detached = fake.detach()

        pred_real = self.netD(real)
        pred_fake = self.netD(fake_detached)
        ...
        gp = self._gradient_penalty(real, fake_detached) * 10.0  
    ...
    def _gradient_penalty(self, real, fake):
        ...
        pred_interpolated = self.netD(interpolated)
    ...
    def _disc_features(self, x, detach=False):
        ...
        (all layers except the final 1-channel conv).
        """
        feats = []
        h = x
    ...
    def compute_feature_matching(self, fake, real):
        """
    ...
    def optimize_parameters(self):
        ...
        # Feature Matching loss on D’s intermediate activations
        ...
        if self.lambda_FM > 0.0:        
            self.loss_FM    = float(loss_FM_tensor.item())
        self.loss_G_total = float(loss_G_total.item())
